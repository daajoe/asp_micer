...
for file in $(find instances_scenario_s2 -name \*.lp -type f -size -5k) ; do
    echo "$file"
    bfile=$(basename $file)
    #gringo --output=text $file expS1_query.lp 
    gringo --output=smodels $file expS1_query.lp 2> $lparse_out/$bfile.grounder_log | ./lp2normal-2.27 1> $lparse_out/$bfile.lparse
...
for file in $(find $lparse_out -type f -name \*.lparse.bz2 ) ; do
    bfile=$(basename $file)
    echo $mus_out/$bfile
    zcat $file | ./lp_parse_choice.py  > $mus_out/$bfile.mus
done
